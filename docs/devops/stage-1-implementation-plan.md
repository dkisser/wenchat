...
| Q4 | Per-message retransmit policy: fixed timeout, exponential backoff, max retries? | **Initial 2 s, exponential ×2, cap at 30 s, give up after 5 retries → emit `outbox-abandoned` event** | Matches heartbeat order of magnitude (2 s ping, 4 s watchdog). 5 × ~30 s ≈ 90 s ceiling; user has long since stopped waiting, so escalation is correct. **Amended 2026-09-21 (ADR 0003):** PR-5's *chunk* retransmit initial delay was raised from 200 ms to 2 s — the fast value stacked duplicate bulk traffic onto SCTP's own congestion recovery and wedged the association (2026-09-21 "transfer completes, connection dies" incident). |
...
| Q9 | Heartbeat vs ACK scheduler: merge or coexist? | **Coexist** | Heartbeat is liveness-only and runs on a separate cadence (2 s ping, 4 s watchdog). ACK is data-flow. Merging them couples liveness to data plane and forces ACK to emit even when idle (which wastes bandwidth). ADR 0001 already calls for coexistence. **Amended 2026-09-21 (ADR 0003):** the watchdog is now re-armed by ANY inbound frame (not just ping/pong) and fires after 15 s of total silence, and pings are suppressed while other traffic flows — the 4 s ping/pong-only watchdog killed healthy mid-transfer connections on congested links. The schedulers still coexist. |
